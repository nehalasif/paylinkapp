...
// --- 1. BANK INSTRUCTIONS ACCORDION COMPONENT (MODIFIED) ---
const BankInstructionsAccordion = ({ consumerId }) => {
  ...
  // Helper function to render a single accordion item
    ...
    <div className="w-full">
      ...
      <div className="mb-6">
          <input
            ...
            className="w-full sm:w-80 p-2 justify-between border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
          />
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
            <div className="bg-slate-50 rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 overflow-y-auto">
                    <BankInstructionsAccordion consumerId={consumerId} />
                </div>
                 <div className="p-4 border-t bg-white rounded-b-lg flex justify-end sticky bottom-0">
                    <button
                        onClick={onClose}
                        className="bg-btnBlue text-white px-5 py-2 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                    >
                        Close
                    </button>
                </div>
...
// --- 3. MAIN PAYMENT COMPONENT (Unchanged) ---
const PaymentInitilization = () => {
   ...
   const backtoHome = () => router.push('/');
    ...
    const CardPayNowOnClick = () => router.push(`/pages/cardinfo`);
    ...
    return (
        <div className="p-1 flex flex-col min-h-screen relative z-10" >
                            ...
                            <div className=" xsize:mr-10">
                                ...
                                <div className="flex items-center" onClick={() => handleToggle(0)}>
                                    <div className="cursor-pointer p-4 flex justify-between items-center w-full">
                                        ...
                                        <div className={`content transform transition-transform ${expanded === 0 ? "rotate-180" : ""}`}>
                                ...
                                <div className="flex items-center" onClick={() => handleToggle(1)}>
                                    <div className="cursor-pointer p-4 flex justify-between items-center w-full">
                                        <div className="content flex items-center">
                                            ...
                                            <span className=' p-4 xsize:pl-0 text-md text-gray-600 xsize:text-sm '> Pay via Raast QR</span>
                                        </div>
                                        <div className={`content transform transition-transform ${expanded === 1 ? "rotate-180" : ""} `}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6"><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div className="flex flex-col justify-center items-center">
                                            <div ref={hiddenRef} className='justify-center lg:w-6/6 mt-1 xsize:w-[90%]' style={{ textAlign: 'center', padding: '10px', backgroundColor: '#F5F7FA', marginTop: '5px', borderRadius: '5px', boxShadow: '0px 0px 6px rgba(0, 0, 0, 0.1)' }}> 
                                                ...
                                                <div className='flex justify-center py-2'>  
                                                    {isQrLoading ? (
                                                        <div className="animate-pulse bg-customPulseColor flex items-center justify-center" style={{ width: '120px', height: '120px', borderRadius: '8px' }}>
                                                            <p className='text-xs'>Fetching QR...</p>
                                                        </div>
                                                    ) : (
                                                        <div style={{ padding: '10px' }}>
                                                            <Canvas text={qRString} options={{ width: 100, margin: 1, bgColor: '#F5F7FA' }} />
                                                        </div>
                                                    )}
                                                </div>
                                            ...
                                            <div className="button pt-3">
                                                <button className="content-white bg-btnBlue border rounded hover:text-btnBlue hover:bg-transparent hover:border-btnBlue text-white px-5 py-2 xsize:text-xs" onClick={handleDownload}>Save To Gallery</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center" onClick={() => handleToggle(2)}>
                                    <div className="cursor-pointer p-4 flex justify-between items-center w-full">
                                        ...
                                        <div className={`content transform transition-transform ${expanded === 2 ? "rotate-180" : ""}`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6"><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
                                        </div>
                                    </div>
                                </div>
                                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${expanded === 2 ? 'max-h-96' : 'max-h-0'}`}>
                                    ...
                                    <div className="flex justify-end p-4">
                                        <button 
                                            onClick={() => setIsHowToPayModalOpen(true)}
                                            className="content-white bg-btnBlue border rounded hover:text-btnBlue hover:bg-transparent hover:border-btnBlue text-white px-5 py-2 xsize:text-xs">
                                            See How to Pay
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="px-4 py-3 xsize:w-full sm:w-full lg:w-12/12">
                                <div className='px-4 py-3 shadow-custom-shadow rounded lg:border md:border xs:border-none border-gray-300'>                                
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button className="content border rounded px-3 py-1 text-md hover:bg-btnBlue hover:text-white">
                                            Download Invoice
                                        </button>
                                    </div>
                                    <div className="border-t mt-5"></div>
                                    <div className="px-2 pt-2 flex justify-center items-center">
                                        <p className="xl:text-2xl uppercase font-normal text-[#505050]">{institutionData.institutionName}</p>
                                    </div>
                                    ...
                                    <div className="px-2 pt-2 flex justify-between items-center">
                                        ...
                                        <p className={`${voucherData.bill_Status === "U" ? "text-red-500" : "text-green-500"} text-sm font-medium`}>
                                    ...
                                    <div className="px-2 pt-5 flex justify-between items-center">
                                        ...
                                        <p className="text-lg text-gray-600">{institutionData.amount_Currency}:{voucherData.billAmount}</p>
                                    </div>
                    ...
                    <div>
                        ...
                        {(!voucherData || !institutionData) ? (
                             <div className="flex flex-col md:flex-row gap-6 p-4 md:p-6 lg:p-8 relative animate-pulse">
                                ...
                                <div className="w-full md:w-6/12 order-1 md:order-2 flex justify-center">
                                <div className="px-4 py-3 w-full lg:w-12/12"><div className="px-4 py-3 shadow-lg rounded-lg border border-gray-200">
                                  <div className="flex justify-between items-center"><div className="h-6 bg-gray-200 rounded w-1/2">
                                  </div><div className="h-8 bg-gray-200 rounded w-1/4"></div></div><div className="border-t mt-5"></div>
                                  <div className="px-2 pt-4 flex justify-center items-center"><div className="h-7 bg-gray-200 rounded w-3/4">
                                  </div></div><div className="space-y-4 mt-4 px-2"><div className="flex justify-between items-center">
                                    <div className="h-4 bg-gray-200 rounded w-1/4"></div><div className="h-4 bg-gray-200 rounded w-1/3">
                                    </div></div><div className="flex justify-between items-center"><div className="h-4 bg-gray-200 rounded w-1/5">
                                    </div><div className="h-4 bg-gray-200 rounded w-1/2"></div></div><div className="flex justify-between items-center">
                                      <div className="h-4 bg-gray-200 rounded w-1/4"></div><div className="h-4 bg-gray-200 rounded w-1/3"></div>
                                      </div>
                                      <div className="flex justify-between items-center"><div className="h-4 bg-gray-200 rounded w-1/6"></div><div className="h-4 bg-gray-200 rounded w-1/4"></div></div></div><div className="border-t mt-5"></div><div className="px-2 pt-5 flex justify-between items-center"><div className="h-5 bg-gray-200 rounded w-1/4"></div>
                                <div className="h-5 bg-gray-200 rounded w-1/3"></div></div></div></div></div>
                            </div>
                        ) : (
                            ...
                            <div className="w-full max-w-lg text-center p-6 bg-white shadow-md rounded-lg border border-gray-200">
                                ...
                                <p className="text-gray-600 mb-4 text-lg">
                                    {voucherData.bill_Status === "P"
                                        ? `This invoice (ID: ${data?.kuickpayID}) has already been paid.`
                                        : `The invoice (ID: ${data?.kuickpayID}) is expired, blocked, or invalid. Please contact the biller for assistance.`}
                                </p>
            ...
            <Footer />
            <HowToPayModal
                isOpen={isHowToPayModalOpen}
                onClose={() => setIsHowToPayModalOpen(false)}
                consumerId={data?.kuickpayID}
            />
        </div>
    );
};
